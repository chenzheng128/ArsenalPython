...
def qbb_qos_init():
    ...
    # 默认使用 OpenFlow13 以支持 mod_ecn
    # ovs-vsctl list bridge s1 | grep protocols #查看s1交换机现有协议
    switch = partial( OVSSwitch, protocols='OpenFlow13' )

    net = Mininet(topo=qbb_topo, switch=switch, controller=None, link=TCLink, autoSetMacs=True, xterms=True, autoStaticArp=True)

    # ovs embed controller of10 only
    # c0 = Controller('c0', port=6633)
    # net.addController(c0)
    # 使用外部控制器以支持 openflow13
    # remote controller to support of13 (like ryu: ./bin/ryu-manager ryu/app/simple_switch_13.py)
