...
############## Begin: Delete Code ###############
        ...
        self.addLink('h1', 's0', bw=bw_net,
                      max_queue_size=maxq )

        for i in xrange(1, n):
            self.addLink('h%d' % (i+1), 's0',
...
def set_q_red(iface, q):
    "Change queue size limit of interface for RED AQM"
    # orignal
    # cmd = ("tc qdisc change dev %s parent 1:1 "
    # ubuntu 14.04
    cmd = ("tc qdisc change dev %s parent 5:1 "
...
def do_sweep(iface):
    ...
    if args.red:
        # set_q_red()
        # inital with red
        cmd = ("tc qdisc del dev %s parent 5:1 "
                % (iface))
        dprint (cmd)
        os.system(cmd)
