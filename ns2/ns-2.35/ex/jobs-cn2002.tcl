...
for {set i 1} {$i <= $N_USERS} {incr i} {
	if {$i <= $N_USERS_TCP} {
    		set cwnd_file($i) [open ./cwnd$i.tr w]
	}core_node
	set thru_file($i) [open ./thru$i.tr w]
}
set thru_file([expr $N_USERS + 1]) [open ./thru_sum.tr w]
...
for {set i 1} {$i < $hops} {incr i} {
	...
	$ns at $START_STATS  "$qmon_xy($i) reset"
      ...
      puts "linkDump [$ns link $core_node($i) $core_node([expr $i+1])] $bing_xy($i) $ping_xy($i) $qmon_xy($i) $STATS_INTR N($i)-N([expr $i+1])"
...
for {set i 1} {$i < $hops} {incr i} {
	set flow_file($i) [open ./flows-hop$i.tr w]
...
for {set i 1} {$i <= $N_USERS} {incr i} {
    	set tot_drop($i) 0
    	set old_arv($i) 0
    	set total_drops_file($i) [open ./totaldrops-flow$i.tr w]
